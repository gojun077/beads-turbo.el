...
(ert-deftest beads-epics-test-goto-issue-opens-standard-detail-view ()
  "RET on an epic should use the same detail navigation flow as list mode."
  (with-temp-buffer
    (beads-epics-mode)
    (beads-epics--render (beads-epics-test--sample-entries))
    (goto-char (point-min))
    ...
    (let ((opened nil)
          (cached nil)
          (rerendered nil)
          (full-issue '((id . "bdel-aaa")
                        (title . "Hydrated epic")
                        (description . "Full details"))))
      (cl-letf (((symbol-function 'beads-cache-get-full-issue)
                 (lambda (_id) nil))
                ((symbol-function 'beads-detail-open)
                 (lambda (issue) (push issue opened)))
                ((symbol-function 'beads-client-show-async)
                 (lambda (_id callback)
                   (funcall callback nil full-issue)))
                ((symbol-function 'beads-cache-put-full-issue)
                 (lambda (id issue)
                   (setq cached (cons id issue))))
                ((symbol-function 'beads-detail-rerender-if-current)
                 (lambda (id issue)
                   (setq rerendered (cons id issue)))))
        (beads-epics-goto-issue))
      (should (equal (alist-get 'id (car opened)) "bdel-aaa"))
      (should (equal cached (cons "bdel-aaa" full-issue)))
      (should (equal rerendered (cons "bdel-aaa" full-issue))))))

(ert-deftest beads-epics-test-goto-issue-uses-cached-full-issue ()
  "Cached full issues should open directly without refetching."
  (with-temp-buffer
    (beads-epics-mode)
    (beads-epics--render (beads-epics-test--sample-entries))
    (goto-char (point-min))
    ...
    (let ((opened nil)
      ...
      (cl-letf (((symbol-function 'beads-cache-get-full-issue)
                 (lambda (_id) full-issue))
                ((symbol-function 'beads-detail-open)
                 (lambda (issue) (setq opened issue)))
                ((symbol-function 'beads-client-show-async)
                 (lambda (&rest _args)
                   (error "unexpected async fetch"))))
        (beads-epics-goto-issue))
      (should (equal opened full-issue)))))
