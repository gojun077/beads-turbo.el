;;; beads-list-data-test.el --- Tests for beads-list-data.el -*- lexical-binding: t; -*-
...
(defun beads-list-data-test--tree-ids (nodes)
  "Return IDS from NODES in preorder."
  (let (ids)
    (dolist (node nodes)
      (push (alist-get 'id (alist-get 'issue node)) ids)
      (setq ids (append (nreverse (beads-list-data-test--tree-ids
                                   (alist-get 'children node)))
                        ids)))
    (nreverse ids)))

(ert-deftest beads-list-data-test-build-applies-filter-marked-sort-and-stats ()
  ...
  (let* ((issues '(((id . "later") (status . "closed") (priority . 1)
    ...
    (should (equal (mapcar (lambda (issue) (alist-get 'id issue))
                           (beads-list-data-issues model))
                   '("open-p2" "open-p0")))
    (should (equal (mapcar (lambda (issue) (alist-get 'id issue))
                           (beads-list-data-display-issues model))
                   '("open-p0")))
    (should (= 3 (alist-get 'total_issues (beads-list-data-stats model))))
    (should (= 1 (alist-get 'closed_issues (beads-list-data-stats model))))))

(ert-deftest beads-list-data-test-find-by-id-and-section ()
  "Lookup and section classification work without renderer state."
  (let ((issues '(((id . "a") (status . "open"))
                  ((id . "b") (status . "blocked"))
                  ((id . "c") (status . "closed")))))
    (should (equal (alist-get 'status (beads-list-data-find-by-id issues "b"))
                   "blocked"))
    (should-not (beads-list-data-find-by-id issues "missing"))
    (should (= 0 (beads-list-data-issue-section (car issues))))
    (should (= 1 (beads-list-data-issue-section (cadr issues))))
    (should (= 2 (beads-list-data-issue-section (caddr issues))))))

(ert-deftest beads-list-data-test-open-issue-with-dependencies-is-blocked-section ()
  "Open issues with incomplete blockers sort into the blocked section."
  (should (= 1 (beads-list-data-issue-section
                '((id . "blocked-open")
                  (status . "open")
                  (dependency_count . 2))))))

(ert-deftest beads-list-data-test-flat-issues-to-forest-roots-and-nested-children ()
  ...
  (let* ((issues '(((id . "root") (title . "Root"))
         ...
         (forest (beads-list-data-flat-issues-to-forest issues))
         (root (car forest))
    ...
    (should (equal (alist-get 'id (alist-get 'issue
                   ...
                   "grandchild"))
    (should (equal (beads-list-data-test--tree-ids forest)
                   '("root" "child" "grandchild" "other")))))

(ert-deftest beads-list-data-test-flat-issues-to-forest-missing-parent-is-root ()
...
(ert-deftest beads-list-data-test-flat-issues-to-forest-prevents-duplicates ()
...
(ert-deftest beads-list-data-test-flat-issues-to-forest-cycle-nodes-are-roots ()
...
(ert-deftest beads-list-data-test-flat-issues-to-forest-filtered-subset-orphans-child ()
  ...
  (let* ((issues '(((id . "parent") (status . "closed"))
         ...
         (open-only (beads-list-data-apply-filter
                     issues (beads-filter-not-closed)))
         (forest (beads-list-data-flat-issues-to-forest open-only))
         (issue (alist-get 'issue (car forest))))
    (should (equal (beads-list-data-test--tree-ids forest) '("child")))
    (should (equal (alist-get 'parent issue) "parent"))))

(provide 'beads-list-data-test)
;;; beads-list-data-test.el ends here
