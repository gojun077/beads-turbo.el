...
| =o= | ~beads-list-cycle-sort~       | Cycle sort columns         |
...
issue trees; column mode preserves the fetched issue order.
...
* Form Editor
:PROPERTIES:
:DESCRIPTION: VUI-based issue editing
:END:
#+cindex: form editor
#+findex: beads-form-vui-mode
...
#+vindex: beads-type-style

#+begin_src emacs-lisp
;; Full names (default): bug, feature, task, epic, chore
(setq beads-type-style 'full)
...
actions, forms) and whenever you return to a =beads-org-list-mode=
...
- ~beads-status-open~ - Open issues
- ~beads-status-in-progress~ - In-progress issues
- ~beads-status-closed~ - Closed issues
- ~beads-priority-p0~ - P0 (critical) - inverted error colors
- ~beads-priority-p1~ - P1 (high) - error colors
...
(set-face-attribute 'beads-priority-p0 nil
                    :background "dark red"
                    :foreground "white"
                    :weight 'bold)
#+end_src
...
- ~beads-org-list-mode-hook~
- ~beads-detail-vui-mode-hook~
- ~beads-form-vui-mode-hook~
...
(add-hook 'beads-detail-vui-mode-hook #'outline-minor-mode)
#+end_src
