...
:END:
#+cindex: list view
#+findex: beads-org-list-mode
#+findex: beads-list-legacy

The list view is your main interface.  Open it with =M-x beads-list=.
By default this opens a generated org-mode task/tree view: issues are
org headings, parent/child relationships are nested, and metadata lives
in a =:PROPERTIES:= drawer.  If you need the old tabulated UI during the
transition, run =M-x beads-list-legacy=.
...
| =n=   | ~org-next-visible-heading~ | Next visible issue  |
| =p=   | ~org-previous-visible-heading~ | Previous visible issue |
| =TAB= | ~org-cycle~              | Fold or unfold subtree |
...
| =o= | ~beads-list-cycle-sort~       | Cycle columns in legacy table |
| =O= | ~beads-list-reverse-sort~     | Reverse legacy table sort  |

*Sectioned sorting* (default) groups by status then sorts by priority.
In the org view, sectioned sorting renders section headings and nested
issue trees; column mode preserves the fetched issue order without table
headers.  Column cycling with =o= and =O= is available in
=M-x beads-list-legacy=.
...
| =H= | ~beads-org-list-hierarchy-show~ | Show dependency tree |
| =S= | ~beads-stats~          | Show statistics          |
| =D= | ~beads-org-list-delete-issue~ | Delete issue at point |
| =R= | ~beads-org-list-reopen-issue~ | Reopen closed issue   |
...
These settings apply to the fallback table opened with
=M-x beads-list-legacy=.  The default org view stores metadata in
property drawers instead of columns.

Control which legacy table columns appear:
...
actions, forms) and whenever you return to a =beads-org-list-mode= or
=beads-list-mode= window from another buffer (for example, closing a
detail view).
...
Temporary text field edit buffers also run ~beads-edit-mode-hook~ when they are
opened from a list, detail, or form edit action.
...
| Command              | Description              |
|----------------------+--------------------------|
| =M-x beads-list=     | Open org issue list      |
| =M-x beads-list-legacy= | Open legacy table list |
| =M-x beads=          | Open org issue list      |
...
| =E=   | Open form editor               |
|-------+--------------------------------|
| =e *= | Edit field (* = t/s/p/T/d/D/a/n/A/x) |
| =e l a= | Add label                    |
| =e l r= | Remove label                 |
